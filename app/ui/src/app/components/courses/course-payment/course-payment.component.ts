...
setDiscount(){
  ...
  this.discountPrice=parseFloat((this.course.price-(this.course.price*this.discountPercentage)/100).toFixed(2))
 ...
 this.couponCode=""
  ...
  initiatePayment() {
    ...
    this.paymentService.createOrder(amount).subscribe((order: any) => {
      if (this.courseCode)
        this.paymentService.initiatePayment(order.id, amount, this.courseCode,this.couponCode);
    });
  }
}
